...
--[+ +ABILITIES+ +]--
	...
	local ED_PSY_Ability_0_rgb_ru = iu_actit(table.concat({
		ppp______ppp,
		"- Может использоваться для предотвращения самоподрыва псайкера.",
	...
	local ED_PSY_Ability_1_3_rgb_ru = iu_actit(table.concat({
		ppp______ppp,
		"- Количество зарядов Горения души, применяемых к врагам, зависит от уровня опасности:",
		"_______________________________",
		"Заряды: 1|      2|        3|       4|       5|       6",
		...
		"- Урон от Горения души увеличивается за счёт пробивания и хрупкости, а также за счёт характеристик текущего оружия и усилений от Талантов: «Прерывание судьбы», «Эмпирейское усиление», «Пагубный импульс», «Кинетическое присутствие», «Идеальный момент» и «Всадник варпа».",
	...
	local ED_PSY_Ability_2_2_rgb_ru = iu_actit(table.concat({
		...
		"- Щит получает 8 урона за каждое прямое попадание от специалистов и исчезает после 3 \"заблокированных\" специалистов. Соблюдает окно восстановления урона в 0,33 секунды, что означает, что любое количество прямых попаданий в тело от Specials, которые происходят в течение 0,33 секунд друг от друга, считается всего лишь 1 полученным прямым попаданием в тело. The shield takes 8 Damage per direct body hit from Specials, disappearing after 3 \"blocked\" Specials. Respects the 0.33 seconds Damage Cooldown window which means that any amount of direct body hits from Specials that happen within 0.33 seconds of each other count as just 1 direct body hit taken.",
		"",
